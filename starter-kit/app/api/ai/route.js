...
//
// 前端（page.jsx）把使用者的煩惱 + 想用哪一頂帽子 + 這頂帽子之前聊過的對話紀錄
// 送到這裡 → 這裡照那頂帽子的角色設定、連同歷史紀錄一起去問 Groq → 把答案送回前端
//
...
如果對話已經有前情，請根據前面聊過的內容繼續補充，不要重複講過的事。
...
如果對話已經有前情，請根據前面聊過的內容繼續補充、講更深入或更多細節，不要重複講過的事。
用繁體中文回答，條列呈現，簡短扼要，不超過 5 點。`,
...
如果對話已經有前情，請根據前面聊過的內容繼續補充新點子，不要重複講過的事。
...
// 一次最多帶幾則過去的對話紀錄一起送 —— 太多會拖慢速度、燒更多 token
const MAX_HISTORY_MESSAGES = 8; // 8 則 = 4 輪一問一答
...
export async function POST(request) {
  // 1. 拿到前端送來的東西：煩惱內容 + 要用哪一頂帽子 + 這頂帽子之前的對話紀錄
  const { input, hat, history } = await request.json();
  ...
  // 3. 整理歷史紀錄：只留 user / assistant 這兩種角色、內容要是字串，並且只留最近幾則
  ...
  // 第一次開口才用「我的煩惱是：」開場，接著聊就直接送原話，比較像真的在對話
  const userContent = priorMessages.length === 0 ? `我的煩惱是：${input}` : input;

  // 4. 呼叫 Groq，用這頂帽子對應的角色設定 + 之前聊過的內容
  try {
